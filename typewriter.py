...
sounds = listdir(getcwd() + '/typewriter_sounds')
sound_tmp = {}
sound_tmp["click"] = []
for sound in sounds:
    if sound == 'enter.wav':
    ...
    elif sound == 'shift.wav':
        sound_tmp["shift"] = sound
    else:
        ...
        if event.type == "EV_KEY" and event.value == 1:
            if event.code.startswith("KEY"):
                if event.code == "KEY_ENTER":
                    filename = getcwd() + '/typewriter_sounds/' + sounds["enter"]
                    print filename
                elif event.code == "KEY_SPACE":
                    filename = getcwd() + '/typewriter_sounds/' + sounds["space"]
                    print filename
                    ...
                    filename = getcwd() + '/typewriter_sounds/' +\
